...
class OrderService
{
     ...
     * @arr 必须要带有主键数组;
     */
    public static function SetOrder($arr)
    {
        //判断是否改成未支付
        ...
        if(Db::name('Order')->where('id',$arr['id'])->value('status') > $arr['status']){
            return DataReturn('违规操作', -1);
        }
        Db::name('order')->update($arr);
        return DataReturn('操作成功', 0);
    }
    public static function OrderDel($params)
    {
        Db::name('order')->delete($params['id']);
        $logs = [
            'admin_id'=>$params['admin']['id'],
            'title'=>'删除',
            'detail'=>$params['admin']['username'].'删除了订单：'.$params['id'],
            'add_time'=>time()
        ];
     ...
     *
     */
    public static function GetOneOrder($oid)
    {
    ...
    public static function OrderListWhere($params = [])
    {
        ...
        {
            $where[] = ['order_no|receive_tel', 'like', '%'.$params['keywords'] . '%'];
        }
        
        //售后服务
        if (!empty($params['service']) && $params['service']==1) {
                $where[] = ['service','=',1];
            }
    ...
    public static function OrderList($params = [])
    {
        ...
        if(!empty($data))
        {
            ...
            foreach($data as &$v)
            {
                ...
                //收货地址
                $v['receive'] = Db::name('Order')->where('id',$v['id'])->value('receive_address_id');
                if($v['receive'] > 0){
                    $v['receive'] = Db::name('UserAddress')->where('id',$v['receive'])->select();
                    $v['receive'] = $v['receive'][0];
                    $v['receive']['province'] = Area::GetAreaName($v['receive']['province']);
                    $v['receive']['city'] = Area::GetAreaName($v['receive']['city']);
                    $v['receive']['county'] = Area::GetAreaName($v['receive']['county']);
                    $v['receive']['tel'] = Area::GetAreaName($v['receive']['phone']);
                }
    ...
    public static function OrderDelivery($params = [])
    {
        ...
        //获取地址信息
        $address_id = Db::name('Order')->where('id',$params['id'])->value('receive_address_id');
        $params['address'] = Db::name('UserAddress')->where('id',$address_id)->select();
        $params['address'] = $params['address'][0];
        /*
        $params['address']['province'] = Area::GetAreaName($params['address']['province']);
        $params['address']['city'] = Area::GetAreaName($params['address']['city']);
        $params['address']['county'] = Area::GetAreaName($params['address']['county']);
*/
        // 获取订单信息
        $where = ['id'=>intval($params['id']), 'user_id'=>$params['user_id'], 'is_delete_time'=>0, 'user_is_delete_time'=>0];
        $order = Db::name('Order')->where($where)->field('id,status,user_id')->find();
        ...
        if(Db::name('Order')->where($where)->update($upd_data))
        {
            ...
            $prameter = [
                ...
                'logistics_num'=>$params['express_number'],
                'end_province'=>$params['address']['province'],
                'end_city'=>$params['address']['city'],
                'end_district'=>$params['address']['county'],
                'end_address'=>$params['address']['address'],
                'end_people'=>$params['address']['name'],
                'end_phone'=>$params['address']['tel'],
                'remark'=>Db::name('Order')->where('id',$params['id'])->value('user_note'),
                'add_time'=>time()
            ];
            $logistics = Db::name('OrderLogistics')->insert($prameter);
            if(!$logistics){
                Db::rollback();
                return DataReturn('生成物流订单失败', -1);
            }
            // 提交事务
            Db::commit();
