...
class GoodsService
{
    ...
    public static function GoodsFavorList($params = [])
    {
        ...
        //f.goods_id,f.user_id,f.add_time,
        // 获取数据
    ...
    /*public static function UserOrderListWhere($params = [])
    {
        ...
        if(!empty($params['keywords']))
        {
            $where[] = ['title|order_no', 'like', '%'.$params['keywords'].'%'];
        }
        return $where;
    }
    public static function UserOrderListWheres($params = [])
    {
        $where = [
            ['o.is_delete_time', '=', 0]
        ];
        ...
        if(!empty($params['user']))
        {
            $where[] = ['og.user_id', '=', $params['user']['id']];
        }
        //状态查询
        if(!empty($params['status'])){
        ...
        if(!empty($params['keywords']))
        {
            $where[] = ['g.title|o.order_no', 'like', '%'.$params['keywords'].'%'];
        }
        return $where;
    }*/
     ...
     * 搜索查询我的订单总数
     */
    public static function SouOrderTotal($where = [])
    {
        return Db::name("order_goods")->alias("og")->join("goods g","g.id = og.goods_id","left")->join("order o","o.id=og.order_id","left")->where($where)->count();
        //return Db::name("order_goods")->where($where)->count();
    }

    /**
     * 我的订单列表
     */
    public static function MyOrderList($params = [])
    {
        $where = empty($params['where']) ? [] : $params['where'];
        $m = isset($params['m']) ? intval($params['m']) : 0;
        $n = isset($params['n']) ? intval($params['n']) : 10;
        ...
        $data = Db::name("order")->alias("o")->field("o.id,o.order_no,o.status,o.add_time,u.nickname,p.name")->join("user u","u.id = o.user_id","left")->join("payment p","p.id = o.payment_id","left")->order($order_by)->limit($m,$n)->where($where)->select();
        foreach($data as $key => $val){
            $data[$key]['goods_son'] = Db::name("order_goods")->alias("og")->field($field)->join("goods g","g.id = og.goods_id")->where(["og.order_id"=>$val['id']])->select();
        }
     ...
     * 我的商品评价列表
     */
    public static function MyGoodsEvaluateList($params = [])
    {
        $where = empty($params['where']) ? [] : $params['where'];
        $m = isset($params['m']) ? intval($params['m']) : 0;
        $n = isset($params['n']) ? intval($params['n']) : 10;
        $order_by = empty($params['order_by']) ? 'o.id desc' : $params['order_by'];
        $field = "g.id as goods_id,g.images,g.price,g.title,og.goods_number";
        // 获取数据
        $array = Db::name("order")
            ->alias("o")
            ->field("o.id,o.order_no,p.name,u.nickname")
            ->join("payment p","p.id = o.payment_id","left")
            ->join("user u","u.id = o.user_id","left")
            ->where($where)
            ->select();
        foreach($array as $key => $val){
            $arr = Db::name("order_goods")
                ->alias("og")
                ->field($field)
     ...
     * 商品评价列表条件
     */
    public static function GoodsEvaluateListWhere($params = [])
    {
        $where = [
            ['g.is_delete_time', '=', 0]
        ];
    ...
    public static function UserGoodsBrowseListWhere($params = [])
    {
        ...
        if(!empty($params['time']))
        {
            ...
            $time = $params['time'];
            $where[] = ['b.add_time', '>=', $time];
            //$time = strtotime($params['time'])+3600*24;
            $time = $params['time']+3600*24;
            $where[] = ['b.add_time', '<=', $time];
        }
    ...
    public static function GoodsList($params = [])
    {
        ...
        //$data = Db::name('Goods')->alias("a")->field($field)->join("goods_details b","a.id = b.goods_id","left")->where($where)->order($order_by)->limit($m, $n)->select();
        return self::GoodsDataHandle($params, $data);
    }
     ...
     * 获取一条商品信息
     */
    public static function GoodsOneInfo($goods_id){
        return Db::name('Goods')->where('id',$goods_id)->field(['images','title','title_color'])->limit(1)->find();
    }
    ...
    public static function GoodsSave($params = [])
    {
        ...
        if(empty($params['id']))
        {
            ...
            //var_dump($params);
            if(!empty($params['details_name'])){
                $post = [];
                $count = count($params['details_name']);
                for($i=0;$i<$count;$i++){
                    $post[] = ['goods_id'=>$goods_id,'details_name'=>$params['details_name'][$i],'details_value'=>$params['details_value'][$i]];
                }
                if(!Db::name("goods_details")->insertAll($post)){
                    return DataReturn("参数添加失败");
                }
            }
            //写入日志
            $logs = [
            ...
            $data['upd_time'] = time();
            $params['title'] = Db::name('Goods')->where(['id'=>intval($params['id'])])->value('title');
            //var_dump($params);
            if(Db::name('Goods')->where(['id'=>intval($params['id'])])->update($data))
            {
                $goods_id = $params['id'];
            }
            Db::name("goods_details")->where(["goods_id"=>$params['id']])->delete();
            $post = [];
            $count = count($params['details_name']);
            for($i=0;$i<$count;$i++){
                $post[] = ['goods_id'=>$params['id'],'details_name'=>$params['details_name'][$i],'details_value'=>$params['details_value'][$i]];

            }
            if(!Db::name("goods_details")->where("goods_id",$params['id'])->insertAll($post)){
                return DataReturn("参数修改失败");
            }
            $logs = [
                'admin_id'=>$params['admin']['id'],
                'title'=>'修改',
                'detail'=>$params['admin']['username'].'修改了商品：'.$params['title']."信息",
                'add_time'=>time()
            ];
            System::AdminLogSave($logs);
        }
    ...
    private static function GoodsSaveBaseUpdate($params, $goods_id)
    {
        ...
        // 更新商品表
        $data['upd_time'] = time();
        $params['title'] = Db::name('Goods')->where(['id'=>$goods_id])->value('title');
        if(Db::name('Goods')->where(['id'=>$goods_id])->update($data))
        {
            $logs = [
                'admin_id'=>$params['admin']['id'],
                'title'=>'修改',
                'detail'=>$params['admin']['username'].'修改了商品: '.$params['title']." 的信息",
                'add_time'=>time()
            ];
    ...
    private static function GetFormGoodsPhotoParams($params = [])
    {
        ...
        if(!empty($params['photo']) && is_array($params['photo']))
        {
            foreach($params['photo'] as $v)
            {
                ...
                $result[] = $v; //ResourcesService::AttachmentPathHandle($v);
            }
        }
    ...
    public static function GoodsDelete($params = [])
    {
        ...
        // 开启事务
        Db::startTrans();
        $params['title'] = Db::name('Goods')->where('id',$params['id'])->value('title');
        // 删除商品
        if(Db::name('Goods')->delete(intval($params['id'])))
        {
            ...
            //参数
            if(Db::name("goods_details")->where(['goods_id'=>intval($params['id'])])->delete() === false){
                Db::rollback();
                return DataReturn('参数删除失败', -100);
            }
            ...
            $logs = [
                'admin_id'=>$params['admin']['id'],
                'title'=>'删除',
                'detail'=>$params['admin']['username'].'删除了商品：'.$params['title'],
                'add_time'=>time()
            ];
            System::AdminLogSave($logs);
            return DataReturn('删除成功', 0);
     ...
     * 多选删除
     */
    public function GoodsAllDel($params = []){

        return DataReturn('123', 2220);
    }
    ...
    public static function GoodsCategorySave($params = [])
    {
        ...
        {
            $data['add_time'] = time();
            $new_id = Db::name('GoodsCategory')->insertGetId($data);
            if($new_id > 0)
            {
                $params['name'] = Db::name('GoodsCategory')->where('id',$new_id)->value('name');
                $logs = [
                    'admin_id'=>$params['admin']['id'],
                    'title'=>'添加',
                    'detail'=>$params['admin']['username'].'添加了商品分类：'.$params['name'],
                    'add_time'=>time()
                ];
                $log = System::AdminLogSave($logs);
                return DataReturn('添加成功', 0);
            ...
            $data['upd_time'] = time();
            $params['name'] = Db::name('GoodsCategory')->where(['id'=>intval($params['id'])])->value('name');
            if(Db::name('GoodsCategory')->where(['id'=>intval($params['id'])])->update($data))
            {
                $logs = [
                    'admin_id'=>$params['admin']['id'],
                    'title'=>'修改',
                    'detail'=>$params['admin']['username'].'修改了商品分类'.$params['name']."信息",
                    'add_time'=>time()
                ];
                $log = System::AdminLogSave($logs);
    ...
    public static function GoodsCategoryDelete($params = [])
    {
        ...
        $params['name'] = Db::name('GoodsCategory')->where(['id'=>$ids])->value('name');
        // 开始删除
        if(Db::name('GoodsCategory')->where(['id'=>$ids])->delete())
        {
            $logs = [
                'admin_id'=>$params['admin']['id'],
                'title'=>'删除',
                'detail'=>$params['admin']['username'].'删除了商品：'.$params['name'],
                'add_time'=>time()
            ];
            $log = System::AdminLogSave($logs);
            return DataReturn('删除成功', 0);
