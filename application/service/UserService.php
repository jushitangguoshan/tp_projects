...
class UserService
{
    ...
    public static function UserSave($params = [])
    {
        ...
        // 更新数据
        $data = [
            'surname'               => isset($params['surrname']) ? $params['surrname'] :  '',
        ...
        if(!empty($params['id']))
        {
            ...
            $data['upd_time'] = time();
            $params['username'] = Db::name('User')->where(['id'=>$params['id']])->value('username');
            if(Db::name('User')->where(['id'=>$params['id']])->update($data))
            {
                if(!empty($params['admin'])){
                    ...
                    $logs = [
                        ...
                        'title'=>'修改',
                        'detail'=>$params['admin']['username'].'修改了用户: '.$params['username']." 的信息",
                        'add_time'=>time()
                    ];
            ...
            $repe = Db::name('User')->where('email',$params['email'])->find();
            if(!empty($repe)){
                return DataReturn('邮件重复', -2);
            }
            $data['add_time'] = time();
            $user_id = Db::name('User')->insertGetId($data);
            $params['username'] = Db::name('User')->where('id',$user_id)->value('username');
            if(!empty($params['admin'])){
                $logs = [
                    'admin_id'=>$params['admin']['id'],
                    'title'=>'添加',
                    'detail'=>$params['admin']['username'].'添加了新用户: '.$params['username'],
                    'add_time'=>time()
                ];
                $log = System::AdminLogSave($logs);
            }
    ...
    public static function UserDelete($params = [])
    {
        ...
        $username = Db::name("User")->where(['id'=>$params['id']])->field("username")->select();
        $params['id'] = empty($username['usernmae']) ? $params['id']  :$username['username'];
        $params['username'] = Db::name('User')->where('id',$params['id'])->value('username');
        // 删除操作
        if(Db::name('User')->delete(intval($params['id'])))
        {
            $logs = [
                'admin_id'=>$params['admin']['id'],
                'title'=>'删除',
                'detail'=>$params['admin']['username'].'删除了用户: '. $params['username'],
                'add_time'=>time()
            ];
            $log = System::AdminLogSave($logs);
    ...
    /*public static function UserAddressList($params = [])
    {
        ...
        // 获取用户地址
        $field = 'id,country,name,phone,province,city,county,gw_city,gw_county,address,is_default';
        $data = Db::name("user_address")->where($where)->field($field)->order('id desc')->select();
        if(!empty($data))
        {
            foreach($data as &$v) {
                ...
                $v['country_name'] = RegionService::countryName($v['country']);
            }
        }
        //var_dump($data);
        return DataReturn('操作成功', 200, $data);
    }*/

    /**
     * 收货地址默认第一（数组套数组问题解决）
     */
    public static function UserAddressLists($params = [])
    {
        // 请求参数
        $p = [
            [
        ...
        //var_dump($where);
        // 获取用户地址
        $field = 'id,user_id,country,name,phone,province,postal_code,city,county,gw_city,gw_county,address,is_default';
        //$data = Db::name("user_address")->where($where)->field($field)->order('is_default desc')->select();
        $arr = Db::name("user_address")->where($where)->field($field)->order('id desc')->select();
        $arr[0]['country_name'] = Db::name("country")->where("id",$arr[0]["country"])->value("country_name");
        $where['is_default'] = 0;
        $array = Db::name("user_address")->where($where)->field($field)->order('id desc')->select();
        foreach($array as $key => $val){
            $array[$key]['country_name']  = Db::name("country")->where("id",$arr[0]["country"])->value("country_name");
        }
    ...
    /**
     * [UserAddressSave 国外用户地址保存]
     * @param   [array]          $params [输入参数]
     */
    public static function UserAbroadAddressSave($params = [])
    {
        // 请求参数
        $p = [
            [
        ...
        $data = [
            ...
            'name'          => $params['name'],
            'phone'           => $params['phone'],
            'gw_city'          => $params['gw_city'],
            'gw_county'        => $params['gw_county'],
            'postal_code'        => $params['postal_code'],
            'address'       => $params['address'],
            'is_default'    => $is_default,
        ];
    ...
    public static function Login($params = [])
    {
        ...
        // 登录帐号格式校验
        if(!CheckMobile($params['email']) && !CheckEmail($params['email']))
        {
        ...
        // 获取用户账户信息
        $where = array('mobile|email' => $params['email'], 'is_delete_time'=>0);
        ...
        if(Db::name('User')->where(array('id'=>$user['id']))->update($data) !== false)
        {
            ...
            if(self::UserLoginRecord($user['id']))
            {
                ...
                return DataReturn('登录成功', 0,$user['id']);
            }
        }
