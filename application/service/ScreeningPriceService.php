...
class ScreeningPriceService
{
        ...
        if(empty($params['id']))
        {
            ...
            if(Db::name('ScreeningPrice')->insertGetId($data) > 0)
            {
                $logs = [
                    ...
                    'title'=>'添加',
                    'detail'=>$params['admin']['username'].'添加了范围：'.$data['name'],
                    'add_time'=>time()
                ];
            ...
            if(Db::name('ScreeningPrice')->where(['id'=>intval($params['id'])])->update($data))
            {
                $logs = [
                    'admin_id'=>$params['admin']['id'],
                    'title'=>'修改',
                    'detail'=>$params['admin']['username'].'修改了范围：'.$data['name']."的信息",
                    'add_time'=>time()
                ];
                $log = System::AdminLogSave($logs);
    ...
    public static function ScreeningPriceDelete($params = [])
    {
        ...
        $data['name'] = Db::name('ScreeningPrice')->where(['id'=>intval($params['id'])])->value('name');
        // 开始删除
        if(Db::name('ScreeningPrice')->where(['id'=>intval($params['id'])])->delete())
        {
            $logs = [
                'admin_id'=>$params['admin']['id'],
                'title'=>'删除',
                'detail'=>$params['admin']['username'].'删除了范围：'.$data['name'],
                'add_time'=>time()
            ];
            $log = System::AdminLogSave($logs);
