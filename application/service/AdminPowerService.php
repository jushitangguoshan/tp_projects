...
class AdminPowerService
{
    ...
    public static function PowerDelete($params = [])
    {
        ...
        $params['name'] = Db::name('Power')->where('id',$params['id'])->value('');
        if(Db::name('Power')->delete(intval($params['id'])))
        {
            ...
            $logs = [
                ...
                'title'=>'删除',
                'detail'=>$params['admin']['username'].'删除了权限：'.$params['name'],
                'add_time'=>time()
            ];
    ...
    public static function RoleDelete($params = [])
    {
        ...
        // 开启事务
        Db::startTrans();
        $params['name'] = Db::name('Role')->where('id',$params['id'])->value('name');
        // 删除角色
        if(Db::name('Role')->delete(intval($params['id'])) !== false && Db::name('RolePower')->where(['role_id'=>intval($params['id'])])->delete() !== false)
        {
            ...
            $logs = [
                'admin_id'=>$params['admin']['id'],
                'title'=>'删除',
                'detail'=>$params['admin']['username'].'删除了角色：'.$params['name'],
                'add_time'=>time()
            ];
            $log = System::AdminLogSave($logs);
            return DataReturn('删除成功', 0);
    ...
    public static function PowerMenuInit()
    {
        ...
        if(($role_id > 0 || $admin_id == 1) && empty($admin_left_menu))
        {
            ...
            if($admin_id == 1 || $role_id == 1)
            {
                ...
                $admin_left_menu = Db::name('Power')->where(['is_show'=>1])->where(array('pid' => 0))->field($field)->order('sort')->select();
            } else {
