...
class PaymentService
{
    ...
    {
        $payment = Db::name('Payment')->field("id,name")->select();
        return $payment;
    }
    /**
     * 初始化
    ...
    public static function PaymentList($params = [])
    {
        ...
        $data = Db::name('Payment')->where($where)->field('id,icon,name,sort,payment,config,apply_terminal,apply_terminal,element,is_enable,is_open_user')->order('sort asc')->select();
...
php;
    ...
    /**
     * 新增/编辑支付方式
     * @param array $params参数
     */
    public static function SavePay($params = []){
        // 请求参数
        $p = [
            [
                'checked_type'      => 'length',
        ...
        $data = [
            'name'                  => $params['name'],
            'icon'                  => $attachment['data']['icon'],
            'sort'                  => isset($params['sort']) ? intval($params['sort']) : 99,
            'is_enable'             => isset($params['is_enable']) ? intval($params['is_enable']) : 0,
            'add_time'              =>time()
        ];
        ...
        if(empty($params['id']))
        {
            ...
            if(Db::name('Payment')->insertGetId($data) > 0)
            {
                $logs = [
                    ...
                    'title'=>'添加',
                    'detail'=>$params['admin']['username'].'添加了支付方式： '.$data['name'],
                    'add_time'=>time()
                ];
            ...
            $data['upd_time'] = time();
            $data['name'] = Db::name('Payment')->where(['id'=>intval($params['id'])])->value('name');
            if(Db::name('Payment')->where(['id'=>intval($params['id'])])->update($data))
            {
                $logs = [
                    'admin_id'=>$params['admin']['id'],
                    'title'=>'修改',
                    'detail'=>$params['admin']['username'].'修改了支付方式： '.$data['name'],
                    'add_time'=>time()
                ];
                return DataReturn('编辑成功', 0);
            }
